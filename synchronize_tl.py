...
def route_sync_traffic_lights(program_time):
    if 'nyc_tl_synchronized{}.net.xml'.format(program_time) in os.listdir("./net_files"):
        return 'net_files/nyc_tl_synchronized{}.net.xml'.format(program_time)

    print("Making synchronized program for program_time={}".format(program_time))
    ...
    #		this should keep the phase pattern intact to prevent conflicts
        ...
        if totalduration != program_time:
            for d in range(0,len(durations)):
                durations[d] = round(durations[d] / totalduration * program_time)
            if sum(durations) != program_time:
                durations[len(durations)-1] = durations[len(durations)-1] + program_time - sum(durations)
            k = 0
            for phase in tl.findall('phase'):
    ...
    tree.write('net_files/nyc_tl_synchronized{}.net.xml'.format(program_time), encoding='utf-8', xml_declaration=True)
    return 'net_files/nyc_tl_synchronized{}.net.xml'.format(program_time)
